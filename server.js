...
//encodes the body from buffer to JSON (?)
//it will parse the data in the request and make it available to you when you do req.body
...
//app.post = a post wast sent in buffer format but was parse to JSON through the body-parser module.
//now the blog post is in JSON

//read the file posts.json and then-
  fs.readFile(__dirname + '/data/posts.json', function(error, file){

//parse it from JSON to a 'normal' js data
    var parsedFile = JSON.parse(file);
    var time = Date.now(); //get the date now
    var newPost = request.body; //save the post sent into newPost variable

    parsedFile[time] = newPost.blogpost; //give the 'key' in the K/V pair a timestamp and save the new post into it

    updatedBlog = JSON.stringify(parsedFile); //make it from a js file back into a JSON format

    fs.writeFile(__dirname + '/data/posts.json', updatedBlog, function (error) { //write the file into the post.js file
